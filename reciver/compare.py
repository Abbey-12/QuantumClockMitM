...
# Read the CSV files
x1, y1 = read_csv('/home/abebu/SimQN/security/QuantumClockMitM/reciver/correlation_data_20240826_115931.csv')
x2, y2 = read_csv('/home/abebu/SimQN/security/QuantumClockMitM/reciver/correlation_data_20240826_115954.csv')

# Find the estimated offset
estimated_offset_normal = x1[np.argmax(y1)] * time_bin
estimated_std_normal = np.std(y1)

# Find the estimated offset
estimated_offset_attacked = x2[np.argmax(y2)] * time_bin
...
plt.plot(np.array(lags_scaled1[window_start:window_end]) * 1e-4, y1[window_start:window_end], 
         label=f'Normal Offset: {estimated_offset_normal:.6f}s\nNormal Std: {estimated_std_normal:.6f}',  color='blue')
plt.plot(np.array(lags_scaled2[window_start:window_end]) * 1e-4, y2[window_start:window_end],
         
         label=f'Attacked Offset: {estimated_offset_attacked:.6f}s\nAttacked Std: {estimated_std_attacked:.6f}', color='red')

# Adding titles and labels
plt.xlabel('Lag (ms)', fontdict=font_properties)  # Replace with your actual X-axis label
plt.ylabel('Normalized Correlation', fontdict=font_properties)  # Replace with your actual Y-axis label
plt.legend()
# plt.grid(True)
